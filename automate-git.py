...
def run(command_line, working_dir, depot_tools_dir=None, output_file=None):
    ...
    sys.stdout.write(
        '-------- Running "' + command_line + '" in "' + working_dir + '"...' + "\n"
    )
    ...
    if "patterns" in config:
        ...
        for entry in config["patterns"]:
            ...
            if result["out"] != "":
                write_msg = True
                re_exclude = (
                    re.compile(entry["exclude_matches"])
                    if "exclude_matches" in entry
                    else None
                )
...
parser.add_option(
    "--url",
    dest="url",
    help="CEF download URL. If not specified the default URL " + "will be used.",
    default="",
)
parser.add_option(
    "--chromium-url",
    dest="chromiumurl",
    help="Chromium download URL. If not specified the default " + "URL will be used.",
    default="",
)
parser.add_option(
    "--checkout",
...
if (
    (options.nochromiumupdate and options.forceupdate)
    or (options.nocefupdate and options.forceupdate)
    or (options.nobuild and options.forcebuild)
    or (options.nodistrib and options.forcedistrib)
    or ((options.forceclean or options.forcecleandeps) and options.fastupdate)
):
...
if (
    options.noreleasebuild
    and (
        options.minimaldistrib
        or options.minimaldistribonly
        or options.clientdistrib
        or options.clientdistribonly
    )
) or (
    options.minimaldistribonly + options.clientdistribonly + options.sandboxdistribonly
    > 1
):
    print("Invalid combination of options.")
    parser.print_help(sys.stderr)
...
if options.clientdistrib or options.clientdistribonly:
    ...
    if options.buildtarget.find(client_app) == -1:
        print(
            "A client distribution cannot be generated if --build-target "
...
if not branch_is_master:
    ...
    if int(cef_branch) < 3071:
        print(
            "The requested branch (%s) is too old to build using this tool. "
...
if not branch_is_3945_or_newer and (
    ...
    print(
        "Python 3 is not supported with branch 3904 and older "
...
if platform == "mac" and not (options.x64build or options.arm64build):
    print(
        "32-bit MacOS builds are not supported. "
...
if options.resave and (options.forcepatchupdate or discard_local_changes):
    print(
        "--resave cannot be combined with options that modify or discard " + "patches."
    )
    parser.print_help(sys.stderr)
    sys.exit(1)
...
if platform == "windows":
    ...
    if options.dryrun and not os.path.exists(git_exe):
        sys.stdout.write(
            "WARNING: --dry-run assumes that depot_tools"
...
if not options.nodepottoolsupdate and "depot_tools_checkout" in build_compat_versions:
    ...
    run(
        ...
        % (
            git_exe,
            "--force " if discard_local_changes else "",
            depot_tools_compat_version,
        ),
        depot_tools_dir,
        depot_tools_dir,
    )
...
if not os.path.exists(gclient_file) or options.forceconfig:
    ...
    gclient_spec = (
        ...
        + ("True" if options.withpgoprofiles else "False")
        + ", "
        + "}, "
        + "'custom_deps': {"
        + "'build': None, "
...
if not options.nochromiumupdate and os.path.exists(chromium_src_dir):
    ...
    chromium_checkout_changed = (
        chromium_checkout_new
        or force_change
...
if chromium_checkout_changed:
    ...
    run(
        "gclient sync %s--nohooks --with_branch_heads --jobs 16"
...
if options.forcepatchupdate or (
    (chromium_checkout_new or not options.fastupdate)
    and chromium_checkout_changed
    and chromium_checkout != chromium_compat_version
):
...
if not options.nobuild and (
    chromium_checkout_changed
    or cef_checkout_changed
    or options.forcebuild
    or not out_src_dir_exists
):
    # Building should also force a distribution.
    options.forcedistrib = True

    # Make sure the GN configuration exists.
    if not options.dryrun and not os.path.exists(os.path.join(cef_src_dir, "BUILD.gn")):
    ...
    for key in os.environ.keys():
        if (
            key.startswith("CEF_")
            or key.startswith("GCLIENT_")
            or key.startswith("GN_")
    ...
    if not options.nodebugbuild:
        ...
        if platform in sandbox_lib_platforms:
            ...
            if os.path.exists(os.path.join(chromium_src_dir, build_path)):
                ...
                run(
                    command + build_path + " cef_sandbox",
                    chromium_src_dir,
                    depot_tools_dir,
                    os.path.join(
                        download_dir, "build-%s-debug-sandbox.log" % (cef_branch)
                    )
                    if options.buildlogfile
                    else None,
                )
        ...
        if platform in sandbox_lib_platforms:
            ...
            if os.path.exists(os.path.join(chromium_src_dir, build_path)):
                ...
                run(
                    ...
                    depot_tools_dir,
                    os.path.join(
                        download_dir, "build-%s-release-sandbox.log" % (cef_branch)
                    )
                    if options.buildlogfile
                    else None,
                )
    ...
    msg(
        "Not building. The source hashes have not changed and "
...
if not options.nodistrib and (
    chromium_checkout_changed or cef_checkout_changed or options.forcedistrib
):
