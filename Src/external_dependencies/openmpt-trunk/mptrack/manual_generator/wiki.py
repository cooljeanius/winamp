...
style = (
    urlopen(
        base_url
        + "/load.php?debug=false&lang=en&modules=mediawiki.legacy.commonPrint%2Cshared%7Cmediawiki.page.gallery.styles%7Cmediawiki.skinning.interface%7Cskins.vector.styles%7Csite.styles&only=styles&skin=vector"
    )
    .read()
    .decode("UTF-8")
)
...
toc_page = (
    urlopen(base_url + "/index.php?title=Manual:_CHM_TOC&action=render")
    .read()
    .decode("UTF-8")
)
...
[WINDOWS]
OpenMPT=,"OpenMPT Manual.hhc",,"""
    + localurl(pages[0])
    + """,,,,,,0x42520,215,0x300e,[20,20,780,580],0xb0000,,,,,,0
...
for p in pages:
    content = (
        urlopen(base_url + "/index.php?title=" + p + "&action=render")
        .read()
        .decode("UTF-8")
    )
    ...
    content = re.sub(
        r'"/images/thumb/(\w+)/(\w+)/([^\/]+?)/([^\/]+?)"', replace_images, content
    )
    ...
    content = re.sub(
        r'<a href="' + base_url_regex + '/(Manual:.+?)"', fix_internal_links, content
    )
    ...
    <h1>"""
        + title(p)
        + '</h1><div id="content" class="mw-body">'
        + content
        + "</div></body></html>"
    )
...
toc_text = re.sub(r"</div>", "", toc_text, flags=re.DOTALL)
toc_text = re.sub(
    r'<a href="' + base_url_regex + '/(.+?)".*?>(.+?)</a>', toc_parse, toc_text
)
...
if (
    subprocess.call(["../../build/tools/htmlhelp/hhc.exe", '"html/OpenMPT Manual.hhp"'])
    != 1
):
