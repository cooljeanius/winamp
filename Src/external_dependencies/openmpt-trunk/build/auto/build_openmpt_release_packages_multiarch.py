...
openmpt_zip_x86_legacy_basepath = "installer/OpenMPT-" + \
    openmpt_version + "-x86-legacy/"
openmpt_zip_amd64_basepath = "installer/OpenMPT-" + openmpt_version + "-amd64/"
openmpt_zip_amd64_legacy_basepath = "installer/OpenMPT-" + \
    openmpt_version + "-amd64-legacy/"
...
def copy_pluginbridge(from_path, arch, to_path):
    ...
    copy_file(from_path + arch + "/", to_path,
              "PluginBridgeLegacy-" + arch + ".exe")
...
def copy_other(to_path, openmpt_version_short):
    ...
    copy_file("packageTemplate/", to_path,
              "OpenMPT Support and Community Forum.url")
...
remove_file("installer/" + openmpt_version_name +
            "-portable-x86-legacy.update.json")
remove_file("installer/" + openmpt_version_name + "-portable-amd64.zip")
remove_file("installer/" + openmpt_version_name +
            "-portable-amd64.update.json")
remove_file("installer/" + openmpt_version_name + "-portable-amd64-legacy.zip")
remove_file("installer/" + openmpt_version_name +
            "-portable-amd64-legacy.update.json")
...
remove_file("installer/" + openmpt_version_name +
            "-portable-x86-legacy.zip.digests")
remove_file("installer/" + openmpt_version_name +
            "-portable-amd64.zip.digests")
remove_file("installer/" + openmpt_version_name +
            "-portable-amd64-legacy.zip.digests")
remove_file("installer/" + openmpt_version_name + "-portable-arm.zip.digests")
remove_file("installer/" + openmpt_version_name +
            "-portable-arm64.zip.digests")
...
signToolCmdLine = ["signtool", "sign", "/fd", "sha256",
                   "/a", "/tr", "http://time.certum.pl", "/td", "sha256"]
...
if (signBinaries):
    ...
    signDirs = [sourcePathModern + "x86/", sourcePathLegacy + "x86/", sourcePathModern + "amd64/",
                sourcePathLegacy + "amd64/", sourcePathModern + "arm/", sourcePathModern + "arm64/"]
    signFiles = []
    for dir in signDirs:
...
copy_symbols(sourcePathLegacy + "x86/",
             openmpt_zip_symbols_path + "x86-legacy/")
copy_symbols(sourcePathModern + "amd64/", openmpt_zip_symbols_path + "amd64/")
copy_symbols(sourcePathLegacy + "amd64/",
             openmpt_zip_symbols_path + "amd64-legacy/")
...
copy_symbols_pluginbridge(sourcePathModern + "x86/",
                          openmpt_zip_symbols_path + "x86/", "x86")
copy_symbols_pluginbridge(sourcePathLegacy + "x86/",
                          openmpt_zip_symbols_path + "x86-legacy/", "x86")
copy_symbols_pluginbridge(sourcePathModern + "amd64/",
                          openmpt_zip_symbols_path + "amd64/", "amd64")
copy_symbols_pluginbridge(sourcePathLegacy + "amd64/",
                          openmpt_zip_symbols_path + "amd64-legacy/", "amd64")
copy_symbols_pluginbridge(sourcePathModern + "arm/",
                          openmpt_zip_symbols_path + "arm/", "arm")
copy_symbols_pluginbridge(sourcePathModern + "arm64/",
                          openmpt_zip_symbols_path + "arm64/", "arm64")
...
if signInstaller:
    isccParams = ["/Ssigntool=" +
                  (" ".join(signToolCmdLine)) + " $f", "install-multi-arch-signed.iss"]

pInno = Popen([pathISCC] + isccParams, cwd="installer/")
if singleThreaded:
    pInno.communicate()

p7zx86 = Popen([path7z, "a", "-tzip", "-mx=9", "../" + openmpt_version_name +
               "-portable-x86.zip", "."], cwd=openmpt_zip_x86_basepath)
if singleThreaded:
    p7zx86.communicate()
p7zx86legacy = Popen([path7z, "a", "-tzip", "-mx=9", "../" + openmpt_version_name +
                     "-portable-x86-legacy.zip", "."], cwd=openmpt_zip_x86_legacy_basepath)
if singleThreaded:
    p7zx86legacy.communicate()
p7zamd64 = Popen([path7z, "a", "-tzip", "-mx=9", "../" + openmpt_version_name +
                 "-portable-amd64.zip", "."], cwd=openmpt_zip_amd64_basepath)
if singleThreaded:
    p7zamd64.communicate()
p7zamd64legacy = Popen([path7z, "a", "-tzip", "-mx=9", "../" + openmpt_version_name +
                       "-portable-amd64-legacy.zip", "."], cwd=openmpt_zip_amd64_legacy_basepath)
if singleThreaded:
    p7zamd64legacy.communicate()
p7zarm = Popen([path7z, "a", "-tzip", "-mx=9", "../" + openmpt_version_name +
               "-portable-arm.zip", "."], cwd=openmpt_zip_arm_basepath)
if singleThreaded:
    p7zarm.communicate()
p7zarm64 = Popen([path7z, "a", "-tzip", "-mx=9", "../" + openmpt_version_name +
                 "-portable-arm64.zip", "."], cwd=openmpt_zip_arm64_basepath)
if singleThreaded:
    p7zarm64.communicate()

p7zsymbols = Popen([path7z, "a", "-t7z", "-mx=9", "../" + openmpt_version_name +
                   "-symbols.7z", "."], cwd=openmpt_zip_symbols_basepath)
if singleThreaded:
