...
class MacroCollector:
    ...
    _define_directive_re = re.compile(
        r"\s*#\s*define\s+(\w+)" + r"(?:\s+|\((\w+)\)\s*)" + r"(.+)"
    )
    ...
    def _make_key_type_from_curve_code(builder, tester):
        return KEY_TYPE_FROM_CURVE_TEMPLATE % {
            "builder": builder,
    ...
    def _make_key_type_from_group_code(builder, tester):
        return KEY_TYPE_FROM_GROUP_TEMPLATE % {
            "builder": builder,
            "builder_length": len(builder),
            "tester": tester,
        }
    ...
    def _make_algorithm_from_hash_code(builder, tester):
        return ALGORITHM_FROM_HASH_TEMPLATE % {
            "builder": builder,
            "builder_length": len(builder),
            "tester": tester,
        }
    ...
    def _make_key_usage_code(self):
        return "\n".join(
            [self._make_bit_test("usage", bit) for bit in sorted(self.key_usages)]
        )
    ...
    def write_file(self, output_file):
        ...
        data["key_type_code"] = (
            self._make_ecc_key_type_code() + self._make_dh_key_type_code()
        )
