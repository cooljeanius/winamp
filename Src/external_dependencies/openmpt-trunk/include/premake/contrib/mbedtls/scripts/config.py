...
def include_in_crypto(name):
    ...
    if (
        name.startswith("MBEDTLS_X509_")
        or name.startswith("MBEDTLS_SSL_")
        or name.startswith("MBEDTLS_KEY_EXCHANGE_")
    ):
        return False
    if name in [
...
class ConfigFile(Config):
    ...
    default_path = [
        ...
        os.path.join(
            os.path.dirname(os.path.abspath(os.path.dirname(__file__))), _path_in_tree
        ),
    ]
    ...
    def __init__(self, filename=None):
        ...
        if filename is None:
            for candidate in self.default_path:
                ...
                raise Exception(
                    "Mbed TLS configuration file not found", self.default_path
                )
        super().__init__()
    ...
    _section_line_regexp = (
        r"\s*/?\*+\s*[\\@]name\s+SECTION:\s*" + r"(?P<section>.*)[ */]*"
    )
    _config_line_regexp = re.compile(
        r"|".join([_define_line_regexp, _section_line_regexp])
    )
