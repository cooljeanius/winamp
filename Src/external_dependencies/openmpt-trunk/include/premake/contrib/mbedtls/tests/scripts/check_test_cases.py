    ...
    * Individual test cases in ssl-opt.sh.
            ...
            for line_number, line in enumerate(data_file, 1):
                ...
                if not in_paragraph:
                    ...
                    self.process_test_case(
                        descriptions, data_file_name, line_number, line
                    )
                in_paragraph = True
    ...
    def walk_ssl_opt_sh(self, file_name):
        ...
        with open(file_name, "rb") as file_contents:
            for line_number, line in enumerate(file_contents, 1):
                ...
                # function name.
                m = re.match(rb'\s*run_test\s+"((?:[^\\"]|\\.)*)"', line)
                if not m:
                    continue
                description = m.group(1)
                self.process_test_case(
                    descriptions, file_name, line_number, description
                )
    ...
    def walk_all(self):
        ...
        for directory in test_directories:
            for data_file_name in glob.glob(
                os.path.join(directory, "suites", "*.data")
            ):
...
def main():
    ...
    if (results.warnings or results.errors) and not options.quiet:
        sys.stderr.write(
            "{}: {} errors, {} warnings\n".format(
                sys.argv[0], results.errors, results.warnings
            )
        )
