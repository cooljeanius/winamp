...
class read_file_lines:
    ...
    def __enter__(self):
        self.generator = enumerate(open(self.filename, "rb" if self.binary else "r"))
        return self
...
class Inputs:
    ...
    def parse_header_line(self, line):
        ...
        if re.search(self._excluded_name_re, name) or name in self._excluded_names:
            return
    ...
    def add_test_case_line(self, function, argument):
        ...
        if function.endswith("_algorithm"):
            ...
            if function == "key_agreement_algorithm" and argument.startswith(
...
def run_c(type_word, expressions, include_path=None, keep_c=False):
    ...
    try:
        c_fd, c_name = tempfile.mkstemp(
            prefix="tmp-{}-".format(type_word), suffix=".c", dir="programs/psa"
        )
        ...
        c_file.write(
            "/* Generated by test_psa_constant_names.py for {} values */".format(
                type_word
            )
        )
        c_file.write(
...
"""
        ...
        subprocess.check_call(
            [cc] + ["-I" + dir for dir in include_path] + ["-o", exe_name, c_name]
        )
        if keep_c:
...
class Tests:
    ...
    def run_one(self, inputs, type_word):
        ...
        output = subprocess.check_output([self.options.program, type_word] + values)
        ...
        for expr, value, output in zip(expressions, values, outputs):
            ...
            if normalize(expr) != normalize(output):
                self.errors.append(
                    self.Error(
                        type=type_word, expression=expr, value=value, output=output
                    )
                )
    ...
    def report(self, out):
        ...
        for error in self.errors:
            out.write(
                'For {} "{}", got "{}" (value: {})\n'.format(
                    error.type, error.expression, error.output, error.value
                )
            )
...
def main():
    ...
    parser.add_argument(
        "--program", default="programs/psa/psa_constant_names", help="Program to test"
    )
    parser.add_argument(
        "--show", action="store_true", help="Keep the intermediate C file"
    )
    parser.add_argument(
        "--no-show",
        action="store_false",
