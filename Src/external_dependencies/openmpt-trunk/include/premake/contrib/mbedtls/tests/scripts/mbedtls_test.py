...
class TestDataParser:
    ...
    def __parse(self, data_f):
        ...
        for line in data_f:
            ...
            grouped_args = [
                (args[i * 2], args[(i * 2) + 1]) for i in range(int(len(args) / 2))
            ]
            self.tests.append((name, function_name, dependencies, grouped_args))
...
class MbedTlsTest(BaseHostTest):
    ...
    def hex_str_bytes(hex_str):
        ...
        if hex_str[0] != '"' or hex_str[len(hex_str) - 1] != '"':
            raise TestDataParserError(
                "HEX test parameter missing '\"':" " %s" % hex_str
            )
        hex_str = hex_str.strip('"')
        if len(hex_str) % 2 != 0:
            raise TestDataParserError(
                "HEX parameter len should be mod of " "2: %s" % hex_str
            )
    ...
    def run_test(self, name, function_id, dependencies, args):
        ...
        param_bytes, length = self.test_vector_to_bytes(function_id, dependencies, args)
        self.send_kv(
    ...
    def get_result(value):
        ...
        try:
            ...
            ValueError(
                "Result should return error number. " "Instead received %s" % value
            )
    ...
    def on_result(self, _key, value, _timestamp):
        ...
        self.log("{{__testcase_finish;%s;%d;%d}}" % (name, int_val == 0, int_val != 0))
        if int_val != 0:
